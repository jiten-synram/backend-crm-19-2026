...
dashRouter.get('/user', async (req, res, next) => {
  try {
    ...
    const [kpis,repCnt,custCnt,monthly,byStatus,incSummary] = await Promise.all([
      ...
      query(`SELECT s.status, COUNT(l.id) AS cnt, COALESCE(SUM(CASE WHEN o.revenue_countable = 1 THEN COALESCE(o.order_amount, 0) ELSE 0 END), 0 ) AS revenue FROM ( SELECT 'new' AS status UNION ALL SELECT 'in_process' UNION ALL SELECT 'follow_up' UNION ALL SELECT 'cnr' UNION ALL SELECT 'converted' UNION ALL SELECT 'delivered' UNION ALL SELECT 'cancelled' UNION ALL SELECT 'dead') s LEFT JOIN leads l ON l.status = s.status AND l.assigned_to = ? LEFT JOIN orders o ON o.assigned_to = l.assigned_to AND o.revenue_countable = 1 GROUP BY s.status ORDER BY FIELD(s.status, 'new', 'in_process', 'follow_up', 'cnr', 'converted', 'delivered', 'cancelled', 'dead')`,[uid]),
